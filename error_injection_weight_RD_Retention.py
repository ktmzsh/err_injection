...
output_path_dir = os.path.join(current_path, 'RD_retention_csv', PE_count_list[CURRENT_TARGET])
##### Path list end
...
def read_trans_data(trans_path):
    ...
    trans_data = np.loadtxt(trans_path, delimiter=",", encoding='utf_8_sig')
    trans_divide = np.split(trans_data, num_of_data)
...
def main():
    ...
    for current_weight_num in range(8): #0:conv1, ... , 7:deconv4
        for day_cycle in range(16):  #0:day0,...,16:day2.5
            ...
            error_cell_num = int(parameter_num * ARR_total_BER[day_cycle])

            print("weight contains {} params.".format(parameter_num))
            print("Progress... \ncurrently date : {} and T_BER is {}".format(date[day_cycle], ARR_total_BER[day_cycle]))
            print("...almost {} params causes errors.".format(error_cell_num))

            for i in range(parameter_num):
                if (random.random() < ARR_total_BER[day_cycle]):
                    #print(arr[i])
            ...
            output_name = os.path.join(output_path_dir, weight_name_list[current_weight_num]+ '_day' + str(date[day_cycle]) +\
                                       '_' + PE_count_list[CURRENT_TARGET] +extention_csv)
